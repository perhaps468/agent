...
public class SiliconFlowEmbeddingClient implements EmbeddingClient {
    ...
    private List<List<Float>> doEmbedOnce(List<String> slice, ModelTarget target) {
        ...
        if (data == null) {
            throw new ModelClientException(provider() + " Embedding 响应中缺�?data 数组", ModelClientErrorType.INVALID_RESPONSE, null);
        }
        ...
        for (JsonElement el : data) {
            ...
            if (emb == null) {
                throw new ModelClientException(provider() + " Embedding 响应中缺�?embedding 字段", ModelClientErrorType.INVALID_RESPONSE, null);
            }
